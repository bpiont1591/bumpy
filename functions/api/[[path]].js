...
export async function onRequest(context) {
  ...
  if (path === '/api/public-config' && method === 'GET') {
    ...
    if (secretInviteUrl) {
      return json({ inviteUrl: secretInviteUrl, source: 'BOT_INVITE_URL' });
    }

    if (!env.BOT_CLIENT_ID) {
      return json({ error: 'Brak BOT_INVITE_URL lub BOT_CLIENT_ID w sekretach środowiska.' }, 500);
    }
  ...
  if (path === '/api/user-panel-access' && method === 'POST') {
    ...
    if (!guildId || !guildName) {
      return json({ error: 'Wymagane: guildId i guildName.' }, 400);
    }
  ...
  if (path === '/api/panel-link' && method === 'POST') {
    ...
    if (!guildId || !guildName) {
      return json({ error: 'Wymagane: guildId, guildName.' }, 400);
    }
