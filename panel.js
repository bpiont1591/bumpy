...
function showOnboardingAccessForm() {
  statusNode.innerHTML = `
    ✅ Bot został dodany. Wejdź do panelu WWW już teraz:<br/><br/>
    <small>Podaj ID serwera oraz nazwę serwera (tak jak na Discordzie). Po zapisaniu przejdziesz do panelu ustawień reklamy/opisu.</small>
  `;

  const onboardingForm = document.createElement('form');
  onboardingForm.className = 'panel-form';
  onboardingForm.innerHTML = `
    <label>
      ID serwera Discord
      <input name="guildId" type="text" maxlength="64" required placeholder="np. 123456789012345678" />
    </label>

    <label>
      Nazwa serwera
      <input name="guildName" type="text" maxlength="120" required placeholder="Twoja nazwa serwera" />
    </label>

    <button class="btn" type="submit">Przejdź do panelu serwera</button>
    <small>Po konfiguracji panelu użyj na Discordzie: <b>/invite</b> (wybór kanału) oraz <b>/bump</b> (publikacja na stronie głównej).</small>
  `;

  form.classList.add('hidden');
  form.insertAdjacentElement('beforebegin', onboardingForm);
  ...
  onboardingForm.addEventListener('submit', async (event) => {
    ...
    const payload = {
      guildId: onboardingForm.guildId.value.trim(),
      guildName: onboardingForm.guildName.value.trim()
    };
